...
object IntegrationTest extends ZIOSpecDefault:
  ...
  val listPools =
    for
      response <- Request.post(url, Body.fromString(ListPools(account.license).toJson))
      result   <- response.body.asString.flatMap { json =>
  ...
  val listMeasurements =
    for
      response <- Request.post(url, Body.fromString(ListMeasurements(account.license, pool.id).toJson))
      result   <- response.body.asString.flatMap { json =>
  ...
  val listChemicals =
    for
      response <- Request.post(url, Body.fromString(ListChemicals(account.license, pool.id).toJson))
      result   <- response.body.asString.flatMap { json =>
